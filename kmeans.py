...
class Kmeans():
    ...
    def dataCount(self):
        ...
        tem = [int(random.uniform(0, self.numSamples)) for i in range(self.k)]
        ...
        if self.dim != 2:
            print(u"无法绘制2纬图")
            return 1
        mark_value = ['or', 'ob', 'og', 'ok', '^r', '+r', 'sr', 'dr', '<r', 'pr']  # 设置绘图颜色
        if self.k > len(mark_value):
            print(u"选择的类别数太大，请重新确认类别数")
            return 1

        for i in range(self.numSamples):
            markIndex = int(self.signData[i, 0])
            plt.plot(self.dataSet[i, 0], self.dataSet[i, 1], mark_value[markIndex])  # 绘制分类图形
